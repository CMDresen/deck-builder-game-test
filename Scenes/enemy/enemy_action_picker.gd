...
#the idea here is that conditional actions are always played FIRST.
#If no conditional action is available, then pick a random chance-based action.
...
func get_first_conditional_action() -> EnemyAction:
	...
	#get first playable, conditional action
...
#this demo uses weighted probabilities, where each action has a unique weight
