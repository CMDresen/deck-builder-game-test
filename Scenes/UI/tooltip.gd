...
#below variable used to avoid jerkiness in the tooltip's transparency changes
...
func hide_tooltip() -> void:
	...
	if tween:
		tween.kill()
	#wait two seconds before going to hide, in case we hover over another card
	#avoids jerkiness with transparency changes
	#"fade_seconds" is 0.2 constant, see above
